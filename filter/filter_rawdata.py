...
def split_pdc(raw_file_dir, save_path):
    unimatch_num = 0  # record unique match
    null_match_idxs, mul_matches_idxs = [], []  # record not unique match
    data = pd.read_excel(raw_file_dir, header=0)  # read the raw file
    result_to_save = []  # buffer to save data
    final_records_num = 0  # the number of saved samples
    for i in tqdm(range(len(data))):
        pdc_id = data.iloc[i]['PDC ID']
        pdc_smi = data.iloc[i]['clean_smiles']
        peptide_id = data.iloc[i]['Peptide ID']
        peptide_smi = data.iloc[i]['Peptide Smiles']
        linker_id = data.iloc[i]['Linker ID']
        linker_smi = data.iloc[i]['Update Linker']
        payload_id = data.iloc[i]['Payload ID']
        payload_smi = data.iloc[i]['Payload Smiles']
        if pdc_smi != '.':
            pdc_mol = Chem.MolFromSmiles(pdc_smi)
        if peptide_smi != '.':
            peptide_mol = Chem.MolFromSmiles(peptide_smi)
        if linker_smi != '.':
            linker_mol = Chem.MolFromSmiles(linker_smi)
        if payload_smi != '.':
            payload_mol = Chem.MolFromSmiles(payload_smi)
        ...
        # match linker from protac
        if (pdc_mol is None) | (linker_mol is None):
            null_match_idxs.append(i)
            continue
        match = pdc_mol.GetSubstructMatches(linker_mol)
        if not match:
            null_match_idxs.append(i)  # record null match
            continue
        elif len(match) == 1:
        ...
        frag_peptide_fpsmi_buff, frag_payload_fpsmi_buff = [], []  # record fingerprint similarity
        peptide_save_buff, linker_save_buff, payload_save_buff = [], [], []  # record smiles
        # parse the protac with matched linker/linkers
        for one_match in match:
            one_match_atom_idxs = set(one_match)
            # extract bonds between linker and other two frags
            ...
            # split protac into three frags by linker (warhead, linker and e3 ligand)
            RWpdc_mol = Chem.RWMol(pdc_mol)
            for bond_idx in sorted(bonds_to_remove, reverse=True):
                bond = RWpdc_mol.GetBondWithIdx(bond_idx)
                RWpdc_mol.RemoveBond(bond.GetBeginAtomIdx(), bond.GetEndAtomIdx())
            frags = rdmolops.GetMolFrags(RWpdc_mol, asMols=True, sanitizeFrags=False)
            frags = list(frags)
            #assert len(frags) == 3, f"Expected 3 frags, but got {len(frags)}"
            if len(frags) != 3:
                continue
            ...
            # tag unknown frags as peptide or payload           
            get_peptide, get_linker, get_payload = None, None, None  # empty the buffer
            get_frag_peptide_fpsmi, get_frag_payload_fpsmi = None, None  # empty the buffer
            get_frag_peptide_smi, get_frag_payload_smi = None, None
            linker_fp = RDKFingerprint(linker_mol)
            for idx, frag in enumerate(frags):
                frag_fp = RDKFingerprint(frag)
                frag_linker_fpsmi = DataStructs.FingerprintSimilarity(frag_fp, linker_fp)
            ...
            if payload_mol is not None:
                payload_fp = RDKFingerprint(payload_mol)
                frag_fp_1 = RDKFingerprint(frags[0])
                frag_fp_2 = RDKFingerprint(frags[1])
                if DataStructs.FingerprintSimilarity(frag_fp_1, payload_fp) > DataStructs.FingerprintSimilarity(frag_fp_2, payload_fp):
                    get_frag_payload_smi, get_frag_peptide_smi = frags_smi[0], frags_smi[1]
                    frag_payload_fpsmi_buff.append(DataStructs.FingerprintSimilarity(frag_fp_1, payload_fp))
                else:
                    get_frag_payload_smi, get_frag_peptide_smi = frags_smi[1], frags_smi[0]
                    frag_payload_fpsmi_buff.append(DataStructs.FingerprintSimilarity(frag_fp_2, payload_fp))
                frag_peptide_fpsmi_buff.append(float(0)) 
            elif peptide_mol is not None:
                peptide_fp = RDKFingerprint(peptide_mol)
                frag_fp_1 = RDKFingerprint(frags[0])
                frag_fp_2 = RDKFingerprint(frags[1])
                if DataStructs.FingerprintSimilarity(frag_fp_1, peptide_fp) > DataStructs.FingerprintSimilarity(frag_fp_2, peptide_fp):
                    get_frag_payload_smi, get_frag_peptide_smi = frags_smi[1], frags_smi[0]
                    frag_peptide_fpsmi_buff.append(DataStructs.FingerprintSimilarity(frag_fp_1, peptide_fp))
                else:
                    get_frag_payload_smi, get_frag_peptide_smi = frags_smi[0], frags_smi[1]  
                    frag_peptide_fpsmi_buff.append(DataStructs.FingerprintSimilarity(frag_fp_2, peptide_fp))
                frag_payload_fpsmi_buff.append(float(0))
            else:
                continue
            
            if get_frag_peptide_smi and get_frag_payload_smi:
                peptide_save_buff.append(get_frag_peptide_smi)  # save smiles
                linker_save_buff.append(linker_smi)  # save smiles
                payload_save_buff.append(get_frag_payload_smi)  # save smiles
        
        # skip cases where no matches meet the conditions
        if (len(peptide_save_buff))*(len(payload_save_buff)) != 0:
            frag_fpsmi_buff = [frag_payload_fpsmi + frag_peptide_fpsmi for frag_payload_fpsmi, frag_peptide_fpsmi in zip(frag_payload_fpsmi_buff, frag_peptide_fpsmi_buff)]
            idx_from_max_value = frag_fpsmi_buff.index(max(frag_fpsmi_buff))
            peptide_fsave = peptide_save_buff[idx_from_max_value]
            linker_fsave = linker_save_buff[idx_from_max_value]
            payload_fsave = payload_save_buff[idx_from_max_value]
        
            result_to_save.append({
                'PDC ID': pdc_id,
                'PDC Smiles': pdc_smi,
                'Peptide ID': peptide_id,
                'Peptide Smiles': peptide_fsave,
                'Linker ID': linker_id,
                'Linker Smiles': linker_fsave,
                'Payload ID': peptide_id,
                'Payload Smiles': payload_fsave,
            })
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='adc', choices=['protac', 'pdc', 'adc'])
    parser.add_argument('--dest', type=str, required=True)
    args = parser.parse_args()

    # example for run: python filter_rawdata.py --raw_path /PROTACdb-1.xlsx --dest IndexFile/protac.xlsx
    if args.dataset == 'protac':
        ...
        split_pdc(args.raw_path, args.dest)
    else:
